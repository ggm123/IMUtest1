...
void CAN_init()
{
	CAN0 = CanBusInita("can0");       //init can
	CAN1 = CanBusInitb("can1");       //init can
}
...
{
	int ret;
	canBus_ta *canbus_ptra = (canBus_ta *) malloc(sizeof(canBus_ta));
	memset(canbus_ptra, 0, sizeof(canBus_ta));
	pthread_mutex_init(&canbus_ptra->rdwrLocka, NULL);
	struct sockaddr_can addr;
	struct ifreq ifr;
	if ((canbus_ptra->canFd = socket(PF_CAN, SOCK_RAW, CAN_RAW)) < 0) //创建 SocketCAN 套接字
			{
		printf("socket err!\n");
	}
	strcpy(ifr.ifr_name, interface);
	if (-1 == (ioctl(canbus_ptra->canFd, SIOCGIFINDEX, &ifr))) //指定 can设备
			{
		printf("ioctl err!\n");
	}
	...
	if ((bind(canbus_ptra->canFd, (struct sockaddr*) &addr, sizeof(addr))) < 0) //将套接字与 can0 绑定
			{
		printf("bind err!\n");
	}
	return canbus_ptra;
...
{
	int ret;
	canBus_tb *canbus_ptrb = (canBus_tb *) malloc(sizeof(canBus_tb));
	memset(canbus_ptrb, 0, sizeof(canBus_tb));
	pthread_mutex_init(&canbus_ptrb->rdwrLockb, NULL);
	struct sockaddr_can addr;
	struct ifreq ifr;
	if ((canbus_ptrb->canFd = socket(PF_CAN, SOCK_RAW, CAN_RAW)) < 0) //创建 SocketCAN 套接字
			{
		printf("socket err!\n");
	}
	strcpy(ifr.ifr_name, interface);
	if (-1 == (ioctl(canbus_ptrb->canFd, SIOCGIFINDEX, &ifr))) //指定 can设备
			{
		printf("ioctl err!\n");
	}
	...
	if ((bind(canbus_ptrb->canFd, (struct sockaddr*) &addr, sizeof(addr))) < 0) //将套接字与 can0 绑定
			{
		printf("bind err!\n");
	}
	return canbus_ptrb;
...
void CanSendThreada(canBus_ta *can)
{
	...
	if(SPC_SDIO_1616_Outa_flag==0)
	...
	else if(SPC_SDIO_1616_Outb_flag==0)
		{
		...
		else if(can->sendBuffer.can_id == 0x209)
		{
...
void CanSendThreadb(canBus_tb *can)
{
	...
	printf("Transform_Velocity_Data.AGV_Velocity=%d\n",Transform_Velocity_Data.AGV_Velocity);

	printf("Transform_Velocity_Data1.AGV_Velocity=%d\n",Transform_Velocity_Data1.AGV_Velocity);

	printf("Transform_Velocity_Data2.AGV_Velocity=%d\n",Transform_Velocity_Data2.AGV_Velocity);

	printf("Transform_Velocity_Data3.AGV_Velocity=%d\n",Transform_Velocity_Data3.AGV_Velocity);
	...
	if (can->sendBuffer.can_id == 0x03)
		{
		 ...
		 {
			 can->sendBuffer.can_id = 0x201;//驱动器1从车头看是左轮
			 can->sendBuffer.can_dlc = 06;
			 can->sendBuffer.data[0] = 0x0F;
			 can->sendBuffer.data[1] = 0x00;
			 can->sendBuffer.data[2] = 0x00;
			 can->sendBuffer.data[3] =0x40; //Transform_Velocity_Data.AGV_Idata[0]; //每增加1转速增加0.09375r/min  40 06=150r/min0x40;//
			 can->sendBuffer.data[4] =0x06; // Transform_Velocity_Data1.AGV_Idata[0];//每增加1转速增加24r/min 0x06;//
			 can->sendBuffer.data[5] = 0x00;
	 }
	else  if(can->sendBuffer.can_id==0x201)
		 {
			 can->sendBuffer.can_id = 0x203;//驱动器2从车头看是右轮
			 can->sendBuffer.can_dlc = 06;
			 can->sendBuffer.data[0] = 0x0F;//0x06停转;0x86是清除错误报警
			 can->sendBuffer.data[1] = 0x00;
			 can->sendBuffer.data[2] = 0x00;
			 can->sendBuffer.data[3] = 0x40;//Transform_Velocity_Data2.AGV_Idata[0]; //每增加1转速增加0.09375r/min  40 06=150r/min0x40;//
			 can->sendBuffer.data[4] = 0x06; //Transform_Velocity_Data3.AGV_Idata[0];//每增加1转速增加24r/min0x06;//
			 can->sendBuffer.data[5] = 0x00;//反转 0xFF
	 }
	else
	{
...
void CanRecvThreada(canBus_ta *can)
{
	...
	fd_set rset;
	 timeout.tv_sec=0;
	 timeout.tv_usec=5000;//微妙

	FD_ZERO(&rset);
	FD_SET(can->canFd, &rset);

	printf("this is can select\n");
	ret = select(can->canFd + 1, &rset, NULL, NULL, NULL);//非阻塞式ret = select(can->canFd + 1, &rset, NULL, NULL, &timeout);//
	if (ret == 0)
	{
